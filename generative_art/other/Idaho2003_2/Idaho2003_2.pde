...
void setup() {
  ...
  for (int i = 0; i < l_rows; i++) {
    ...
    minus_plus_plus[1][i] = (l_array[1][i] - 215);
    minus_plus_plus[2][i] = (l_array[2][i]);
    
    // --+
    minus_minus_plus[0][i] = (-1 * l_array[0][i] + 50);
    minus_minus_plus[1][i] = (-1 * l_array[1][i] + 250);
    minus_minus_plus[2][i] = (l_array[2][i] + 250);
  }
  ...
  for (int i = 0; i < m_rows; i++) {
    ...
    plus_plus_plus[1][i] = (m_array[1][i] - 215); // should be the y value. We want this to be constant.
    plus_plus_plus[2][i] = (m_array[2][i]);
    
    // ---
    minus_minus_minus[0][i] = (-1 * m_array[0][i]);
    minus_minus_minus[1][i] = ((-1 * m_array[1][i]) + 300);
    minus_minus_minus[2][i] = (-1 * m_array[2][i]);
    
    // ++-
    plus_plus_minus[0][i] = (m_array[0][i]);
    plus_plus_minus[1][i] = (m_array[1][i] - 215);
    plus_plus_minus[2][i] = (-1 * m_array[2][i] + 50);
    
    // +--
    plus_minus_minus[0][i] = (m_array[0][i] + 200);
    plus_minus_minus[1][i] = (-1 * m_array[1][i] + 215);
    plus_minus_minus[2][i] = (-1 * m_array[2][i] + 200);
  ...
  for (int i = 0; i < s_rows; i++) {
    ...
    plus_minus_plus[1][i] = (-1 * s_array[1][i] + 215);
    plus_minus_plus[2][i] = (s_array[2][i] + 200);
    
    // -+-
    minus_plus_minus[0][i] = (-1 * s_array[0][i]);
    minus_plus_minus[1][i] = (s_array[1][i] - 315);
    minus_plus_minus[2][i] = (-1 * s_array[2][i] + 350);
  }
  
  // Random Noise Cubes
  for (int i = 0; i < p_rows; i++) {
    float x = random(0, 850);
...
// TODO ARRANGE THE LARGE CUBES IN A MORE ARTISTIC WAY
